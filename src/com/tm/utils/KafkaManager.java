...
import com.tm.utils.ZJPFileListener;


public class KafkaManager {
	private static Logger logger = Logger.getLogger(KafkaManager.class);
	Object pm;	//spring注入的配置存储类实例
	private ZkClient zkClient=null;
	private static KafkaManager kafkaManager = new KafkaManager();
	public static String STORAGE_ALL="/storage/all/workers";
	public static String STORAGE_TEMP="/storage/temp/workers";
	public static String messageUrl="http://192.168.60.64/wlzx/users/dxSend/GPSerror2";
	public static String oommessageParam="id=3";
	...
	private KafkaManager()
	{
		...
		try
		{
			InputStream in=new BufferedInputStream(KafkaManager.class.getClassLoader().getResourceAsStream("config/application.properties"));
			properties.load(in);
	        ...
	        String tmapp_messageurl=properties.getProperty("tmapp.messageurl");
	        String tmapp_oommessageparam=properties.getProperty("tmapp.oommessageparam");
	        KafkaManager.messageUrl=tmapp_messageurl;
	        KafkaManager.oommessageParam=tmapp_oommessageparam;
	        synchronized(this)
	        {
	        	if(zkClient==null)
	        	{
	...
	public void appBind()
	{
		...
		Thread registerThread=new Thread()
		{
			...
			public void run() {
				super.run();
				kafkaManager.registerStorage(null, null);
			}
		};
	 ...
	 */
	public void registerStorage(String param_index,String param_port)
	{
		String index="";
		String port="";
...
//		String localIp=param_index;
	...
	public static void main(String[] args) throws Exception{
		...
		kafkaManager.registerStorage(kafkaManager.getLocalIp(),"13568");
		for(int i=0;i<100;i++)
		{
			Thread.sleep(1000);
		}
